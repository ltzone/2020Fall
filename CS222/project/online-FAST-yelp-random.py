...
def make_deep_set(tables):
    return [[set(i) for i in sub_table] for sub_table in tables]


def load_service_table(recommendation_list_ori, service_num, user_num, topN):
    service_topN_users = [[] for i in range(topN)]
    for j in range(topN):
        for i in range(service_num):
            service_topN_users[j].append([])
        for i in range(user_num):
            for j in range(j):
                service_temp = recommendation_list_ori[i][j]
                service_topN_users[j][service_temp].append(i)
    ...
    for service_i in range(service_num):
        for user_j in service_topN_users[topN-1][service_i]:
            for i in range(topN):
...
class OnlineFASTSolution:
    def __init__(self, service_num, user_num,
        ...
        self.service_topN_users_set = make_deep_set(self.service_topN_users)
    ...
    def random_assign(self, rec_service, assigned_users, cur_N):
        total_users = self.service_topN_users_set[cur_N][rec_service] - assigned_users[rec_service]
        if len(total_users) == 0 or random.random() < (1 / len(total_users) / self.user_probability):
            return 1
        else:
            return -1
    ...
    def run_random(self):
        ...
        for Round in range(self.recommendation_total_round):
            ...
            Satisfaction = 0
            assigned_users = [set() for i in range(self.service_num)]
            ...
            for (cur_user_index, request) in enumerate(self.requests_list[Round]):
                ...
                while len(recommendation_list[request]) < self.Top_N:
                    ...
                    if service_seat[rec_service] > 0:
                        user_i = request
                        if self.random_assign(rec_service, assigned_users, min(self.Top_N - 1, recommendation_index_ori)) != -1:
                            recommendation_list[user_i].append(rec_service)
                            assigned_users[rec_service].add(request)
            ...
            for service_temp in range(self.service_num):
                if len(self.service_topN_users[self.Top_N -1][service_temp]) == 0:
                    continue
                p_theory_temp = 0
                for user in self.service_topN_users[self.Top_N -1][service_temp]:
...
def generate_requests_dynamic_size_compound(user_probability=None, rounds=100):
    if user_probability is None:
        user_probability = [(400, 0.6), (400, 0.8)]
    requests = [[] for i in range(rounds)]
    user_idx = 0
    for (user_count, probability) in user_probability:
        total_user = user_idx+user_count
        for i in range(rounds):
            users = []
            for j in range(user_idx,total_user):
                if random.random() < probability:
                    users.append(j)
            requests[i] += (random.sample(users, len(users)))
        user_idx += user_count
    return requests
...
if __name__ == "__main__":
    ...
    online_fast_solver = OnlineFASTSolution(
        ...
        loc_results=f'results/[{current_time}]{mode}_phoenix_improved_random.csv',
